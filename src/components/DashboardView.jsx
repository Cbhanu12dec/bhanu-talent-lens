...
import { tailorResume, draftEmail, ocrImages } from '../lib/claude.js';
...
export default function DashboardView({ uid, state, credits, onCreditsChange, notify, goToBilling, goToLibrary }) {
  ...
  function handleRegenerate() {
    const extra = [...pickedSuggestions, ...(quickTweak.trim() ? [quickTweak.trim()] : [])];
    handleTailor(extra);
  }
  ...
  async function handleTailor(extraPrompts = []) {
    ...
    try {
      ...
      if (tailoredResume) setPreviousVersion({ resume: tailoredResume, atsScore, metTarget });
      setTailoredResume(result.resume);
      setAtsScore(result.atsScore ?? null);
      setMetTarget(result.metTarget !== false);
      setQuickTweak(''); setPickedSuggestions([]);
        ...
        <div>
          <div className="panel">
            ...
            <div className="toolbar">
              <button className="btn btn-ghost btn-sm" onClick={() => { setJdText(''); setTailoredResume(null); setAtsScore(null); setMetTarget(true); setPreviousVersion(null); }}>Clear</button>
          ...
          {tailoredResume && (
            <div className="panel">
              <div className="panel-head"><h2>Refine &amp; regenerate</h2><span className="count">optional — uses 1 more credit</span></div>
